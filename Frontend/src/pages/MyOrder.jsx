      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {orders.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">Start shopping for beautiful plants!</p>
            <button
                  ...
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    {order.items.length > 3 && (
                      <div className="w-16 h-16 bg-gray-200 rounded flex items-center justify-center">
                        <span className="text-gray-600 font-semibold">+{order.items.length - 3}</span>
                      </div>
                    )}
                  </div>
                  <div className="flex justify-between items-center">
                    <p className="text-sm text-gray-600">
                      {order.items.length} item(s) • {order.payment.paymentMethod.toUpperCase()}
                    </p>
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white rounded-lg max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-[#274E36]">Order Details</h2>
              ...
              {/* Delivery Address */}
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-semibold mb-2 text-[#274E36]">Delivery Address</h3>
                <p className="text-gray-700">{selectedOrder.address}</p>
              </div>

              {/* Payment Info */}
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-semibold mb-2 text-[#274E36]">Payment Information</h3>
                <div className="space-y-1">
                  <p className="text-gray-700">
                    Method: <span className="font-medium">{selectedOrder.payment.paymentMethod.toUpperCase()}</span>
                  </p>
                  <p className="text-gray-700">
                    Status: <span className={`font-medium ${
                      selectedOrder.payment.paymentStatus === 'paid' ? 'text-green-600' : 'text-yellow-600'
                    }`}>
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between text-xl font-bold">
                  ...
                  <span className="text-[#274E36]">Rs. {selectedOrder.totalAmount.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
